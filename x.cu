...
__global__ void gpuIntConverter(int* d_cells,int d_ret,int d_size)
{
    int tid=threadIdx.x+blockDim.x*blockIdx.x;
    __shared__ int hold;
    if(tid<d_size)
...
__host__ void cudaRNG(int seed,int size,int* rule)
{
    ...
    Cm((int**)&d_res,size*sizeof(int));//temp
    Cm((int**)&d_out,size*sizeof(int));//temp
    ...
    int no_blocks=(size+31)/32;
    for(int i=0;i<(size/2);i++)
    {
        gpuPrinter<<<1,1>>>(d_out,size);
        gpuElementaryCellularAutoRule30<<<no_blocks,32>>>(d_cells,d_out,size,d_rule);

        cudaDeviceSynchronize();

        gpuTransf<<<no_blocks,32>>>(d_out,d_cells,size);
    }
...
int main(int argc, char** argv) 
{
    ...
    Cm((int**)&d_cells,8*sizeof(int));
    Cm((int**)&d_newcells,8*sizeof(int));
    Cm((int**)&d_temp,4*sizeof(int));

    Cmc(d_cells,cells,8*sizeof(int),CmcHD);
    for(int i=0;i<2;i++)
    {

        Cmc(d_cells,cells,8*sizeof(int),CmcHD);
        // gpuElementaryCellularAutoRule30<<<1,8>>>(d_cells,d_newcells,8);
        cudaDeviceSynchronize();
        Cmc(newcells,d_newcells,8*sizeof(int),CmcDH);
        for (int i = 0; i < 8; i++) 
        {
            printf("%c ", newcells[i]);
            cells[i]=newcells[i];
